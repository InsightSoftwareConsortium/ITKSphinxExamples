...
example_idx = example_dir.index('Examples')
...
with tarfile.open(os.path.join(html_output_dir, example_name + '.tar.gz'), \
    ...
    def strip_path(tarinfo):
        ...
        if tarinfo.name[-4:] != '.cxx' and tarinfo.name[-4:] != '.rst' \
                and basename != 'CMakeLists.txt' and basename != 'build':
                    head, tail = os.path.split(tarinfo.name)
                    tarinfo.name = os.path.join(head, 'build', tail)
        return tarinfo
...
with zipfile.ZipFile(os.path.join(html_output_dir, example_name + '.zip'), \
    ...
    for path in files_for_tarball:
        ...
        if arcname[-4:] != '.cxx' and arcname[-4:] != '.rst' \
                and basename != 'CMakeLists.txt' and basename != 'build':
                    head, tail = os.path.split(arcname)
                    arcname = os.path.join(head, 'build', tail)
