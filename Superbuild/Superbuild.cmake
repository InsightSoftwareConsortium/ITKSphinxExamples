...
option(
  USE_GIT_PROTOCOL
  "If behind a firewall turn this off to use http instead."
  OFF
)
...
option(
  ITKSphinxExamples_USE_WRAP_PYTHON
  "Enable downloads of the ITK Python wrapping to the superbuild"
  ${_use_python_default}
)
if(
  NOT
...
if(BUILD_DOCUMENTATION)
  if(ITKDoxygen_DIR)
    set(
      _build_doc_args
      "${_build_doc_args} -DITKDoxygen_DIR:PATH=${ITKDoxygen_DIR}"
    )
  endif()
  if(ITKDoxygenXML_DIR)
    set(
      _build_doc_args
      "${_build_doc_args} -DITKDoxygenXML_DIR:PATH=${ITKDoxygenXML_DIR}"
    )
  endif()
  if(ITKDoxygenTAG_DIR)
    set(
      _build_doc_args
      "${_build_doc_args} -DITKDoxygenTAG_DIR:PATH=${ITKDoxygenTAG_DIR}"
    )
  endif()
  if(DOC_WITH_LOCAL_DOXYGEN)
    set(
      _build_doc_args
      "${_build_doc_args} -DDOC_WITH_LOCAL_DOXYGEN:PATH=${DOC_WITH_LOCAL_DOXYGEN}"
    )
  endif()
...
ExternalProject_Add(
  ITKSphinxExamples
  DEPENDS
    ${ITKSphinxExamples_DEPENDENCIES}
  PREFIX ITKEx-prefix
  DOWNLOAD_COMMAND
    ""
  ...
  CMAKE_GENERATOR ${gen}
  CMAKE_ARGS
    ${ep_common_args} ${_build_doc_args} ${_python_args}
    -DBUILD_SHARED_LIBS:BOOL=FALSE
    # ITK — pass through ITK_DIR if provided, otherwise FetchContent handles it
    -DITK_DIR:PATH=${ITK_DIR} -DBUILD_TESTING:BOOL=${BUILD_TESTING}
